package com.tdd.blog.website.controller;

import com.github.pagehelper.PageInfo;
import com.tdd.blog.website.constant.WebConst;
import com.tdd.blog.website.dto.ErrorCode;
import com.tdd.blog.website.dto.MetaDto;
import com.tdd.blog.website.dto.Types;
import com.tdd.blog.website.exception.TipException;
import com.tdd.blog.website.modal.Bo.ArchiveBo;
import com.tdd.blog.website.modal.Bo.RestResponseBo;
import com.tdd.blog.website.modal.Vo.CommentVo;
import com.tdd.blog.website.modal.Vo.MetaVo;
import com.tdd.blog.website.service.IMetaService;
import com.tdd.blog.website.service.ISiteService;
import com.tdd.blog.website.utils.PatternKit;
import com.tdd.blog.website.utils.TaleUtils;
import com.tdd.blog.website.modal.Bo.CommentBo;
import com.tdd.blog.website.modal.Vo.ContentVo;
import com.tdd.blog.website.service.ICommentService;
import com.tdd.blog.website.service.IContentService;
import com.tdd.blog.website.utils.IPKit;
...
public class IndexController extends BaseController {
    ...
    public RestResponseBo comment(HttpServletRequest request, HttpServletResponse response,
        ...
        if (StringUtils.isBlank(ref) || StringUtils.isBlank(_csrf_token)) {
            return RestResponseBo.failMsg(ErrorCode.BAD_REQUEST);
        }
        ...
        if (null == cid || StringUtils.isBlank(text)) {
            return RestResponseBo.failMsg("请输入完整后评论");
        }

        if (StringUtils.isNotBlank(author) && author.length() > 50) {
            return RestResponseBo.failMsg("姓名过长");
        }

        if (StringUtils.isNotBlank(mail) && !TaleUtils.isEmail(mail)) {
            return RestResponseBo.failMsg("请输入正确的邮箱格式");
        }

        if (StringUtils.isNotBlank(url) && !PatternKit.isURL(url)) {
            return RestResponseBo.failMsg("请输入正确的URL格式");
        }

        if (text.length() > 200) {
            return RestResponseBo.failMsg("请输入200个字符以内的评论");
        }

        String val = IPKit.getIpAddrByRequest(request) + ":" + cid;
        Integer count = cache.hget(Types.COMMENTS_FREQUENCY.getType(), val);
        if (null != count && count > 0) {
            return RestResponseBo.failMsg("您发表评论太快了，请过会再试");
        }
