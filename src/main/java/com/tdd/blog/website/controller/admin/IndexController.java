...
import com.tdd.blog.website.dto.LogActions;
...
import com.tdd.blog.website.modal.Vo.LogVo;
...
import com.tdd.blog.website.utils.GsonUtils;
...
public class IndexController extends BaseController {
    ...
    public RestResponseBo upPwd(@RequestParam String oldPassword, @RequestParam String password, HttpServletRequest request,HttpSession session) {
        ...
        if (StringUtils.isBlank(oldPassword) || StringUtils.isBlank(password)) {
            return RestResponseBo.failMsg("请确认信息输入完整");
        }

        if (!users.getPassword().equals(TaleUtils.MD5encode(users.getUsername() + oldPassword))) {
            return RestResponseBo.failMsg("旧密码错误");
        }
        if (password.length() < 6 || password.length() > 14) {
            return RestResponseBo.failMsg("请输入6-14位密码");
        }
